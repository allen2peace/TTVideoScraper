...
class Scraper:
            ...
            if 'user' in original_url:
                ...
                if js['item_list'][0]['images'] is not None:
                    ...
                    video_data = {'status': 'success',
                                  ...
                                  'desc': video_title,
                                  'share_url': share_url,
                                  'duration': js['item_list'][0]['video']['duration'],
                                  'video_hashtags': video_hashtags,
                                  'original_url': original_url,
                                  'video': {
                                      'without_watermark': {
                                          'url_list': [nwm_video_url_1080p if nwm_video_url_1080p else video_url],
                                          'width': js['item_list'][0]['video']['width'],
                                          'height': js['item_list'][0]['video']['height'],
                                          # 'data_size': result["aweme_list"][0]["video"]['play_addr']['data_size'],
                                      },
                                      'with_watermark': {
                                          'url_list': js['item_list'][0]['video']["play_addr"]["url_list"],
                                          'width': js['item_list'][0]['video']['width'],
                                          'height': js['item_list'][0]['video']['height'],
                                      },
                                      'cover': js['item_list'][0]['video']["cover"],
                                      'dynamic_cover': js['item_list'][0]['video']["dynamic_cover"],
                                      'origin_cover': js['item_list'][0]['video']["origin_cover"],
                                  },
                                  'music': {
                                      'title': js['item_list'][0]["music"]['title'],
                                      'owner_nickname': js['item_list'][0]["music"]['author'],
                                      'duration': js['item_list'][0]["music"]['duration'],
                                      'download_url': js['item_list'][0]["music"]['play_url'],
                                      'cover': {
                                          'cover_hd': js['item_list'][0]["music"]['cover_hd'],
                                          'cover_large': js['item_list'][0]["music"]['cover_large'],
                                          'cover_medium': js['item_list'][0]["music"]['cover_medium'],
                                          'cover_thumb': js['item_list'][0]["music"]['cover_thumb'],
                                      }
                                  },
                                  'author': {
                                      'uid': js['item_list'][0]['author']["uid"],
                                      'nickname': js['item_list'][0]['author']["nickname"],
                                      'unique_id': js['item_list'][0]['author']["unique_id"],
                                      'avatar': {
                                          'avatar_thumb': js['item_list'][0]['author']["avatar_thumb"],
                                          'avatar_medium': js['item_list'][0]['author']["avatar_medium"],
                                          'avatar_larger': js['item_list'][0]['author']["avatar_larger"],
                                      },
                                  }
                                  }
                    return video_data
    ...
    def tiktok(self, original_url):
        ...
        if '@' in original_url:
            ...
            # 从请求头中获取原始链接
            response = requests.get(url=original_url, headers=headers, allow_redirects=True, proxies=self.proxies)
            try:
                true_link = response.headers['Location'].split("?")[0]
            except:
                true_link = response.request.url
        ...
        try:
            ...
            if 'image_post_info' in response:
                ...
                video_data = {'status': 'success',
                              'url_type': url_type,
                              'desc': result["aweme_list"][0]["desc"],
                              'share_url': result["aweme_list"][0]['share_url'],
                              'duration': result["aweme_list"][0]["video"]['duration'],
                              'video_hashtags': video_hashtags,
                              'original_url': original_url,
                              'video': {
                                  'without_watermark': {
                                      'url_list': result["aweme_list"][0]["video"]["play_addr"]["url_list"],
                                      'width': result["aweme_list"][0]["video"]['width'],
                                      'height': result["aweme_list"][0]["video"]['height'],
                                      'data_size': result["aweme_list"][0]["video"]['play_addr']['data_size'],

                                  },
                                  'with_watermark': {
                                      'url_list': result["aweme_list"][0]["video"]["download_addr"]["url_list"],
                                      'width': result["aweme_list"][0]["video"]['width'],
                                      'height': result["aweme_list"][0]["video"]['height'],
                                      'data_size': result["aweme_list"][0]["video"]['download_addr']['data_size'],
                                  },
                                  'cover': result["aweme_list"][0]["video"]["cover"],
                                  'dynamic_cover': result["aweme_list"][0]["video"]["dynamic_cover"]
                              },
                              'music': {
                                  'title': result["aweme_list"][0]["music"]['title'],
                                  'owner_nickname': result["aweme_list"][0]["music"]['owner_nickname'],
                                  'duration': result["aweme_list"][0]["music"]['duration'],
                                  'download_url': result["aweme_list"][0]["music"]['play_url'],
                                  'cover': {
                                      'cover_hd': result["aweme_list"][0]["music"]['cover_hd'],
                                      'cover_large': result["aweme_list"][0]["music"]['cover_large'],
                                      'cover_medium': result["aweme_list"][0]["music"]['cover_medium'],
                                      'cover_thumb': result["aweme_list"][0]["music"]['cover_thumb'],
                                  }
                              },
                              'author': {
                                  'uid': result["aweme_list"][0]['author']["uid"],
                                  'nickname': result["aweme_list"][0]['author']["nickname"],
                                  'region': result["aweme_list"][0]['author']["region"],
                                  'avatar': {
                                      'avatar_thumb': result["aweme_list"][0]['author']["avatar_thumb"],
                                      'avatar_medium': result["aweme_list"][0]['author']["avatar_medium"],
                                      'avatar_168x168': result["aweme_list"][0]['author']["avatar_168x168"],
                                      'avatar_300x300': result["aweme_list"][0]['author']["avatar_300x300"],
                                  },
                              }
                              }
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        vid_list_join = ' '.join(sys.argv[1:])
        vid_lines = vid_list_join.split(' ')

    vid_url = vid_lines[0]
    scraper = Scraper()
    url = re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', vid_url)[0]
    try:
